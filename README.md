...
Current state: **v2.2.0** — every item in the original v3 gap-analysis report is done, skill file is one coherent document (v2.1.0), and a real production bug reported from live use is fixed (v2.2.0). Agent skill file **v2.2.0**, **226/226 tests passing** via `npm test`, real-verified in this environment (18 test files, 226 tests total, re-run 3x consecutively with zero flakiness).

**Real bug fixed this version, reported from live use:** the Gemini adapter sent tool results with `role: 'function'` in the `contents` array — the live Gemini API rejects this outright ("Role 'function' is not supported. Please use a valid role: SYSTEM, ... USER, ... MODEL, ..."), breaking every tool-using turn for Gemini users. Fixed: tool results now go in a `role: 'user'` turn with a `functionResponse` part, matching Gemini's actual current API (only `user`/`model` are valid content roles; `function` was likely valid in an older API version or a different SDK's abstraction, not the raw REST API this code calls directly). Regression test added in `test/ai-providers.test.js` asserting no request content ever uses `role: 'function'` and that the `functionResponse` part lands inside a `user`-role turn.

**Also added this version — a version-consistency guard**, directly addressing the "update/delivery process" gap from the last assessment. `test/helpers/mock-electron.js` now checks `main.js`'s exported `UNIVERSAL_SKILL_VERSION` against an `EXPECTED_LITEIDE_VERSION` constant in the harness itself, and throws a clear, actionable error immediately if they don't match — instead of a confusing `ReferenceError` three test files later (exactly what happened when a user's `main.js` was updated but their `test/subagents.test.js` wasn't, between v1.9.0 and v2.0.0). **This constant must be bumped every time `UNIVERSAL_SKILL_VERSION` bumps** — it's now a checklist item alongside bumping package.json's version.
